 ...
 * Fiel al HTML de referencia (FIP_Alto_Aporte__1_.html).
 * Layout A4 portrait: col izq 68mm (negra), col der contenido.
 */
"use strict";
...
function pct(v, d=2) {
  ...
  if (typeof v==="string") return v;   // ya formateado desde Python
  if (isNaN(v)) return "—";
...
function slide1(pres, d) {
  ...
  // Título: "Liquidez\nActiva" — siempre 2 palabras en nombre_corto
  const words = (d.nombre_corto||"").split(" ");
  const l1 = words.slice(0,-1).join(" ") || words[0];
  ...
  let y=2.0;
  const LW=IZQ-P*2;
  function secL(t){ sl.addText(t,{x:P,y,w:LW,h:0.18,fontSize:8,bold:true,color:C.negro,fontFace:"Calibri",margin:0}); y+=0.2; sl.addShape("line",{x:P,y,w:LW,h:0,line:{color:C.negro,width:1}}); y+=0.07; }
  ...
  // Comentario PM
  sl.addText("Comentario Portafolio Manager",{x:DER,y:yr,w:DER_W,h:0.27,fontSize:13,bold:true,color:C.negro,fontFace:"Calibri",margin:0});
  yr+=0.3; lin(sl,yr,DER,DER_W,C.negro,1.5); yr+=0.1;
  sl.addText(d.comentario||"",{x:DER,y:yr,w:DER_W,h:1.8,fontSize:8,color:C.grisOsc,fontFace:"Calibri",valign:"top",margin:0,wrap:true,align:"justify"});
  yr+=1.9;

  // Gráfico
  const cd = buildChart(d);
  if (cd.labels.length > 0) {
    sl.addText(d.nombre_corto||"",{x:DER,y:yr,w:DER_W,h:0.16,fontSize:7.5,bold:true,color:C.negro,fontFace:"Calibri",align:"center",margin:0});
    yr+=0.17;
    const series=[];
    if(cd.icp.some(v=>v!==null))  series.push({name:"ICP Nom.",labels:cd.labels,values:cd.icp});
    if(cd.fip.some(v=>v!==null))  series.push({name:d.nombre_fip||"FIP",labels:cd.labels,values:cd.fip});
    if(cd.comp.some(v=>v!==null)) series.push({name:"Competencia Relevante (*)",labels:cd.labels,values:cd.comp});
    if (series.length) {
      ...
      sl.addChart("line",series,{
        ...
        catAxisLabelFrequency:Math.max(1,Math.floor(cd.labels.length/10)),
  ...
  sl.addText("Evolución Rentabilidad",{x:DER,y:yr,w:DER_W,h:0.25,fontSize:13,bold:true,color:C.negro,fontFace:"Calibri",margin:0});
  yr+=0.28; lin(sl,yr,DER,DER_W,C.negro,1.5); yr+=0.08;

  // Tabla resumen (3 filas fijas: ICP, Competencia, FIP)
  const r=d.resumen||{};
  const acumLbl=`Acum.\n${d.anio_acum||"2026"} (*)`;
  ...
  const mkH=(t,l=false)=>({text:t,options:{bold:true,fontSize:7.5,fontFace:"Calibri",align:l?"left":"center",color:C.negro,fill:{color:C.blanco},border:bTop}});
  const mkD=(t,l=false,bg="FFFFFF")=>({text:String(t||"—"),options:{fontSize:7.5,fontFace:"Calibri",align:l?"left":"center",color:l?C.negro:C.grisOsc,bold:l,fill:{color:bg},border:bRow}});
  ...
  // Tabla histórica — desde el año inicio del FIP, cabe en el espacio restante
  ...
  if (hist.length > 0) {
    ...
    const mkTH=(t,l=false)=>({text:t,options:{fontSize:5.5,bold:true,fontFace:"Calibri",color:C.negro,align:l?"left":"center",border:bH,wrap:false}});

    // Calcular rowH dinámico para que todo quepa
    const nAños = hist.length;
    const nFilasTotal = 1 + hist.reduce((acc,row)=>
      acc + 1 + (row.comp!==null?1:0) + (row.fip!==null?1:0), 0);
    const espacioDisponible = H - yr - 0.05;
    const rowH = Math.min(0.13, Math.max(0.09, espacioDisponible / nFilasTotal));

    const trows=[[mkTH("Año",true),mkTH("Fondo",true),...mL.map(m=>mkTH(m))]];

    hist.forEach(row=>{
      const icpC="777777", compC="999999";
      // ICP
      trows.push([
        ...
        ...(row.icp||Array(12).fill(null)).map(v=>({text:v!==null?pct(v):"",options:{fontSize:5,fontFace:"Calibri",color:icpC,align:"center",border:bD,wrap:false}})),
        {text:row.icpT!==null&&row.icpT!==undefined?pct(row.icpT):"—",options:{fontSize:5,bold:true,fontFace:"Calibri",color:icpC,align:"center",border:bD,wrap:false}}
      ]);
      // Competencia
      if(row.comp!==null&&row.comp!==undefined){
        trows.push([
          ...
          ...(row.comp||Array(12).fill(null)).map(v=>({text:v!==null?pct(v):"",options:{fontSize:5,fontFace:"Calibri",color:compC,align:"center",border:bD,wrap:false}})),
          {text:row.compT!==null&&row.compT!==undefined?pct(row.compT):"—",options:{fontSize:5,bold:true,fontFace:"Calibri",color:compC,align:"center",border:bD,wrap:false}}
        ]);
      }
      // FIP
      if(row.fip!==null&&row.fip!==undefined){
        trows.push([
          ...
          ...(row.fip||Array(12).fill(null)).map(v=>({text:v!==null?pct(v):"",options:{fontSize:5,fontFace:"Calibri",color:C.negro,align:"center",border:bD,wrap:false}})),
          {text:row.fipT!==null&&row.fipT!==undefined?pct(row.fipT):"—",options:{fontSize:5,bold:true,fontFace:"Calibri",color:C.negro,align:"center",border:bD,wrap:false}}
        ]);
      }
    });
...
function slide2(pres, d) {
  ...
  sl.addText("F O N D O",{x:P,y:1.12,w:IZQ-P*2,h:0.2,fontSize:7,color:"AAAAAA",fontFace:"Calibri",charSpacing:4,margin:0});
  sl.addShape("line",{x:IZQ,y:0,w:0,h:H,line:{color:"EEEEEE",width:0.5}});

  let y=1.6;
  const LW=IZQ-P*2;
  function secL(t){ sl.addText(t,{x:P,y,w:LW,h:0.18,fontSize:8,bold:true,color:C.negro,fontFace:"Calibri",margin:0}); y+=0.2; sl.addShape("line",{x:P,y,w:LW,h:0,line:{color:C.negro,width:1}}); y+=0.08; }
  function divL(){ sl.addShape("line",{x:P,y,w:LW,h:0,line:{color:C.grisLin,width:0.5}}); y+=0.1; }
  ...
  const mon=d.comp_moneda||[]; mon.length?mon.forEach(([n,v])=>rowL(n,v)):rowL("—","—");
  divL();
  secL("Composición por Instrumento");
  const ins=d.comp_instrumentos||[]; ins.length?ins.forEach(([n,v])=>rowL(n,v)):rowL("—","—");
  divL();
  secL("Composición por Duración");
  const dur=d.comp_duracion||[]; dur.length?dur.forEach(([n,v])=>rowL(n,v)):rowL("—","—");

  /* ── Col der: Glosario + Disclaimer (TEXTO FIJO) ──────────────────── */
  let yr=0.15;
  ...
  GL.forEach(([t,desc])=>{
    sl.addText([{text:t,options:{bold:true,breakLine:true}},{text:desc}],
      {x:DER,y:yr,w:DER_W,h:0.43,fontSize:7,color:C.grisOsc,fontFace:"Calibri",valign:"top",margin:0,wrap:true});
    yr+=0.46;
  });
